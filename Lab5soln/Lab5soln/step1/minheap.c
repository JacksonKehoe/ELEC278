...
// Create instance of heap. Parameter size is size of array to allocate.
// Returns pointer to newly created heap storage if memory obtained;
// NULL if there was any failure to obtain memory.
{
	Heap	*h = malloc(sizeof(Heap));
	if (h != NULL)	{
		h->a = malloc(sizeof(int) * size);
		if (h->a == NULL)	{
			// Didn't get memory for array - give up completely
			free (h);
			h = NULL;
		} else	{
