...
// ****************************************************************************
// Define one of the following two manifests. This will have the effect of
// including code that either creates a maxheap or a minheap.
...
// ***************************************************************************
// Side note.  From user point of view, requirement that list be sorted in
// ascending or descending order is met by seeing list displayed in required
// order. Some may realize that descending order can be accomplished by
// printing list sorted in ascending order in reverse order - in other words,
// just by rewriting for-loop above as:
//		for (i = n-1; i <= 0; i--)	printf("%d ", a[i]);
// This prints from high-index to low index, thus displaying list in
// opposite order.
// ****************************************************************************
...
long merge(int *a, int start, int middle, int end, int *b)
{
...
#ifdef ASCENDING_ORDER
		if (a[j] < a[k])	{ b[i] = a[j]; j++; swaps++; }
		else 				{ b[i] = a[k]; k++; swaps++; }
#endif
#ifdef DESCENDING_ORDER
		if (a[j] > a[k])	{ b[i] = a[j]; j++; swaps++; }
		else 				{ b[i] = a[k]; k++; swaps++; }
	...
	while (j <= middle)	{ b[i++] = a[j++]; swaps++; }

	while (k <= end)	{ b[i++] = a[k++]; swaps++; }
...
long radixSort(int *a, int n, int p)
{
	...
	for (k = 0; k < p; k++) {
		...
		for (i = 1; i < 10; i++) {
			offset[i] = 0;
			for (j = 0; j < i; j++) {offset[i] += count[j];}
			}
...
{
	static int initvalues [7] = { 10, 24, 5, 32, 1, 84, 19 };
	int		i;
	for (i=0; i<7; i++)	a[i] = initvalues [i];
}//initarray()
